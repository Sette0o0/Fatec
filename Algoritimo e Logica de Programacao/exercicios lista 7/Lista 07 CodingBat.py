...
# B. string_splosion
# string_splosion('Code') -> 'CCoCodCode' 001012 0123
# string_splosion('abc') -> 'aababc' 001 012
# string_splosion('ab') -> 'aab' 0 01
...
def array_front9(nums):
    lista = []
    count = 0
    while len(lista) != len(nums) and len(lista) < 4:
        lista.append(nums[count])
        count += 1

    nine = False
    count9 = lista.count(9)
    if count9 > 0: nine = True
    return nine
