...
countA = 0
for hinata in txtA:
    if hinata[0] in zmb and hinata[-1] not in zmb:
        countA += 1

print(f"Texto A tem {countA} preposições.")

countB = 0
for ino in txtB:
    if ino[0] in zmb and ino[-1] not in zmb:
        countB += 1

print(f"Texto B tem {countB} preposições.")

verbsA = 0
pVerbA = 0
for tsunade in txtA:
    if len(tsunade) == 7 and tsunade[-1] not in zmb:
        verbsA += 1
        if tsunade[0] not in zmb:
            pVerbA += 1

print(f"Texto A tem {verbsA} verbos, em primeira pessoa são {pVerbA}")

verbsB = 0
pVerbB = 0
for sakura in txtA:
    if len(sakura) == 7 and sakura[-1] not in zmb:
        verbsB += 1
        if sakura[0] not in zmb:
            pVerbB += 1
